import { useGSAP } from '@gsap/react'
import gsap from 'gsap'
import React, { useRef } from 'react'
import { Link } from 'react-router-dom'
import { Links } from 'react-router-dom'

import { useLocation } from 'react-router-dom'
import { FiMenu } from "react-icons/fi";


const NavbarProj = () => {


  let rasta = useLocation().pathname;
  let divRef = useRef(null) ;
  let divRef1 = useRef(null) ;
  let divRef2 = useRef(null) ;


  const smallGreenAnimate = useRef(null)
  const navIcon = useRef(null)
  return (
    <div className='fixed  z-20 w-full  top-0 left-0 flex justify-between items-start '>

      <Link to='/' className=' w-30 m-2 ' >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103 44">
          <path fillRule="evenodd" fill="black" d="M35.1441047,8.4486911 L58.6905011,8.4486911 L58.6905011,-1.3094819e-14 L35.1441047,-1.3094819e-14 L35.1441047,8.4486911 Z M20.0019577,0.000230366492 L8.83414254,25.3433089 L18.4876971,25.3433089 L29.5733875,0.000230366492 L20.0019577,0.000230366492 Z M72.5255345,0.000691099476 L72.5255345,8.44846073 L94.3991559,8.44846073 L94.3991559,16.8932356 L72.5275991,16.8932356 L72.5275991,19.5237906 L72.5255345,19.5237906 L72.5255345,43.9274346 L102.80937,43.9274346 L102.80937,35.4798953 L80.9357483,35.4798953 L80.9357483,25.3437696 L94.3996147,25.3428482 L94.3996147,16.8953089 L102.80937,16.8953089 L102.80937,0.000691099476 L72.5255345,0.000691099476 Z M-1.30398043e-14,43.9278953 L8.78642762,43.9278953 L8.78642762,0.0057591623 L-1.30398043e-14,0.0057591623 L-1.30398043e-14,43.9278953 Z M58.6849955,8.4486911 L43.1186904,43.9274346 L52.3166592,43.9274346 L67.9877996,8.4486911 L58.6849955,8.4486911 Z M18.4688864,25.3437696 L26.7045278,43.9278953 L36.2761871,43.9278953 L28.1676325,25.3375497 L18.4688864,25.3437696 Z"></path>
        </svg>
      </Link>

      <div className='w-2/3 h-35  flex '>
        <Link to='/agence'
          onMouseEnter={()=>{
            divRef.current.style.height = "100%" ;
          }}
          onMouseLeave={()=>{
            divRef.current.style.height = "0" ;
          }}
          className={`w-1/3 h-1/3 relative cursor-pointer  hover:bg-[#D3FD50] hover:text-black  text-2xl text-white font-[font2] bg-black uppercase `}>
            <div ref={divRef} className='bg-[#D3FD50] absolute top-0 duration-500 transition-all left-0 w-full  '></div>
          <h1 className='absolute ml-2 left-0 bottom-0   '>Agence</h1></Link>
          <Link to='/'
          onMouseEnter={()=>{
            divRef1.current.style.height = "100%" ;
          }}
          onMouseLeave={()=>{
            divRef1.current.style.height = "0" ;
          }}
          className={`w-1/3 h-2/3 relative cursor-pointer  hover:bg-[#D3FD50] hover:text-black  text-2xl text-white font-[font2] bg-black uppercase `}>
             <div ref={divRef1} className='bg-[#D3FD50] absolute top-0 duration-500 transition-all left-0 w-full  '></div>
          <h1 className='absolute ml-2 left-0 bottom-0   '>contact</h1></Link>
          <Link to='/nav'
          onMouseEnter={()=>{
            divRef2.current.style.height = "100%" ;
          }}
          onMouseLeave={()=>{
            divRef2.current.style.height = "0%" ;
          }}
          className={`w-1/3 h-full relative cursor-pointer  hover:bg-[#D3FD50] hover:text-black text-2xl text-white font-[font2] bg-black uppercase `}>
             <div ref={divRef2} className='bg-[#D3FD50] absolute top-0 duration-500 ease-linear transition-all left-0 w-full  '></div>
          <h1 className='absolute ml-2 left-0 bottom-0   '>menu</h1></Link>
        </div>
      </div>
  )
}

export default NavbarProj